...
def _add_health(db, health_score, sleep_score, days_ago=0):
    dt = datetime.utcnow() - timedelta(days=days_ago)
    r = HealthRecord(
        created_at=dt, language="English",
...
def _add_emotion(db, stress, energy, days_ago=0):
    dt = datetime.utcnow() - timedelta(days=days_ago)
    r = EmotionRecord(
        created_at=dt, language="English",
...
class TestMetricAnalyzer:
    def test_health_score_improving(self, db_session):
        ...
        result = compute_metric_trend(db_session, "health_score", days=7)
    ...
    def test_stress_improving(self, db_session):
        ...
        result = compute_metric_trend(db_session, "stress", days=7)
        assert result["direction"] == "improving"
    ...
    def test_energy_improving(self, db_session):
        ...
        result = compute_metric_trend(db_session, "energy", days=7)
        assert result["direction"] == "improving"
    ...
    def test_sleep_score_declining(self, db_session):
        ...
        result = compute_metric_trend(db_session, "sleep_score", days=7)
        assert result["direction"] == "declining"
        assert result["higher_is_better"] is False
    ...
    def test_unknown_metric(self, db_session):
        result = compute_metric_trend(db_session, "nonexistent", days=7)
        assert result["direction"] == "unknown"


class TestTrendSummary:
    def test_empty_db(self, db_session):
        result = compute_summary(db_session, days=7)
