"""Tests for the bilingual medical disclaimer and its render function."""

from modules.ui import DISCLAIMER, get_disclaimer_text
...
def test_get_disclaimer_text_returns_english_by_default():
    """get_disclaimer_text returns English by default."""
    text = get_disclaimer_text()
    assert text == DISCLAIMER["English"]


def test_get_disclaimer_text_returns_chinese():
    """get_disclaimer_text returns Chinese when requested."""
    text = get_disclaimer_text("中文")
    assert text == DISCLAIMER["中文"]


def test_get_disclaimer_text_fallback_to_english():
    """get_disclaimer_text falls back to English for unknown languages."""
    text = get_disclaimer_text("Korean")
    assert text == DISCLAIMER["English"]


def test_english_disclaimer_indicates_informational_use():
    """English disclaimer must state informational/educational purpose."""
    text = get_disclaimer_text("English")
